...
3.  Lists all tenants who have never submitted a maintenance request.
...
4.  Lists the vendors who have handled more maintenance requests than the average
...
5.  Lists the properties whose unit occupancy rate falls below the overall company average. The query calculates the number of occupied units and the occupancy percentage for each qualifying property
...
6.  Lists each payment method used by tenants, the total number of transactions made with that method, the number of completed and failed transactions, and the failure rate as a percentage. Results are ordered by failure rate in descending order
...
7.   Lists tenants whose last names match common surname patterns (Clark, Hall, Walker, or Lewis), along with their monthly rent and property name, but only for tenants whose leases are still active (ending on or after April 1, 2024)
...
8.  Lists every tenant with an active lease (ending in 2024 or later), the number of bedrooms
...
9.   Lists the number of leases and the average monthly rent for each bedroom count (1- bedroom, 2-bedroom, 3-bedroom).
...
10. Lists all maintenance requests that are not yet resolved (Open, In Progress, or Urgent), along with the assigned vendor and their service category. Results are sorted by the date the issue was reported 
...
11.  Lists all tenants who own pets, along with the pet's name, type, breed, and the propertythe tenant currently lives in. Only tenants with active leases are included
